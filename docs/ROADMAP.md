...
| R2 | **Referans v2 eşitleme** (`reference/MonArt2_clean/`, farklar `docs/reference-v2-changes.md`). 11b QA'sından önce biter; her alt adım ayrı sohbette yapılabilir, ayrıntı §6 "Faz R2" | ✅ 5/5 alt adım bitti (17.09.2026) |
...
| R2e | Seri seçimine bağlı tipografi | ✅ 17.09.2026. Arayüz tipografisi seriye göre değişmiyor; seriye bağlı tek tipografi sikke yazısı. Mısır notu ve hiyeroglif alt başlığı düzeltildi, konfigüratörün gündüz metin renkleri referansa eşitlendi (yayında doğrulandı). **Ek (17.09.2026, kullanıcı kararı):** Osmanlı yazısı lisanslı AlphaKufi'ye geçti (koda gömülü) ve gündüz renkleri müşterinin değiştirebilmesi için Day şemasındaki slot'lara bağlandı (6 yeni `Configurator/Day …` slot'u + Text, Accent/Light, Text/Ink). Yayında doğrulandı: AlphaKufi yükleniyor ve sağa/sola bakan Osmanlı yazısında çiziliyor (gündüz/gece); gündüz renkleri referansla aynı (bilerek bırakılan ve yakın slot'a bağlanan 4 fark dışında); 9 slot değiştirilince konfigüratör rengi değişiyor; gecede gündüz rengi yok; 400px ve Ürün sayfası (1200/400px) taşmasız. Ayrıntı `docs/typography-audit.md` |
...
**Font ve dil**
- Fontlar Google Fonts adıyla tema tipografi token'ıdır; tema tipografisine yerel font bağlanamaz. **Tek istisna (kullanıcı kararı 17.09.2026, lisans alındı):** canvas'taki Osmanlı yazısı AlphaKufi. Dosya base64 olarak `src/utils/alpha-kufi-font.ts`'e gömülü, `CoinCanvas` `ensureEmbeddedFont()` (`src/utils/fonts.ts`) ile `FontFace` olarak yükler; DOM'da kullanılmaz. `Coin Script` (Reem Kufi) tipografi token'ı 17.09.2026'da editörden silindi.
...
#### R2d — Konfigüratör: portre yönü + zincir + metinler ✅ (17.09.2026)
Yapılanlar (17.09.2026, yeniden yapılmaz):
- Kullanıcı kararları: `consentText` RICH_TEXT'e çevrildi (kalın kısım için; `FaceDesigner` dış `<p>`'yi soyup etiket içine basar). Mobil açıklama bizim yığılma noktamız 860px'te devreye girer (referans 980px).
- `coin.ts`: `Direction`, `ChainLength`, `CHAIN_LENGTHS`; `FaceState.direction` (varsayılan sağ, `defaultFace` → "Temizle" sağa döndürür). Zincir state'i `CoinConfigurator`'da, "Temizle" sıfırlamaz.
- `FaceDesigner`: cinsiyetten sonra "Portre Yönü" etiketi + `fd__info-dot` + `fd__field-sub` + `fd__gender` görünümlü anahtar; `onOpenDirectionInfo` prop'u.
- Yeni `src/sub-components/DirectionInfoModal`: `.dirm` sınıfları, renkler token'dan (panel `--black`/`--bg-page` karışımı), Esc/backdrop/×, açılınca odak ×'e, kapanınca açan düğmeye; `useScrollLock`; `prefers-reduced-motion`.
- `CoinCanvas`: `direction` prop'u; sola bakınca clip içinde `translate(SIZE,0) scale(-1,1)`; Osmanlı `drawSideText(..., side)` sol yayda `cursor = π − 0.95 + spread`, `step = -1`.
- Zincir alanı 2. yüz adımıyla sipariş notu arasında (`.cfg__chain*`, `max-width: 460px`, ≤640px tam genişlik).
- Opsiyon senkronu: `OPTION_CONTRACT.direction = "Portre Yönü"`, `chain = "Zincir"`; yön "sola"/"saga", zincir "50"/"55"/"60" anahtarıyla; 2. yüz kapalıyken Yüz 2 yönü boş.
- Sepet/sipariş özeti: sola bakan yüzün meta parçasına opsiyon değeri ("Sola Bakan Profil"), ayrı parça olarak "<opsiyon adı>: <değer>" (Zincir Uzunluğu: 55 cm) — metinler ikas'tan, Header/CartPage'e prop eklenmedi. `LineDesign.direction`; küçük resim `lineThumbFor()` → `{ src, mirrored }`, `MIRROR_STYLE` inline `scaleX(-1)` (CartLine, OrderLine, OrderCard, RefundRequest). `order-line.ts` `lineThumb` artık nesne döner.
- Prop'lar (CLI): `head.descriptionMobile`; `faces.direction{Label,Sub,Left,Right,InfoAria}`; yeni grup `chain` (`chainLabel`, `chainSub`, `chain50Text`/`55`/`60`); yeni grup `dirModal` (`dirModalTitle`, `dirModalLead`, `dirModalKicker`, `dirModalSub`, `dirClause1-4Title/Text`, `dirModalCloseLabel`); `options.optDirection`, `options.optChain`. Varsayılan güncellemeleri: `consentText` (RICH_TEXT), `consentMissingToast`, `cancelInfoBody` (bizde "Kişiye özel … m. 15/1-b" cümlesi hiç yoktu, v2 metniyle eklendi; `consentInfoBody` değişmedi).
- ≤640px'te yön düğmeleri `fd__gender--dir` ile dar harf aralığı (iki satıra iniyordu).
- Editör: `import_section` CoinConfigurator, Header, CartPage, AccountPage. Yeni prop'lar import'ta varsayılanla doldu; `consentText`, `consentMissingToast`, `cancelInfoBody` `CnxvAPawqP` ve `vCT6WKmBTD`'ye yazıldı.
...
#### R2e — Seri seçimine bağlı tipografi ✅ (17.09.2026; sonuçlar ve yayın testi `docs/typography-audit.md` sonunda)
