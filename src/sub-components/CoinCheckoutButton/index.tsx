...
interface Props {
  /** Buton metni. Sondaki ":)" ayrı bir span'a alınır ve hover'da döner. */
  text: string;
  disabled?: boolean;
  busy?: boolean;
  className?: string;
...
/** Metnin sonundaki gülümsemeyi ayırır: "Sikke Sikke Öde :)" → ["Sikke Sikke Öde", ":)"]. */
function splitSmile(text: string): [string, string] {
  const m = text.match(/^(.*?)\s*(:\)|☺|🙂)\s*$/);
  return m ? [m[1], m[2]] : [text, ""];
}

/** Referansın "Sikke Sikke Öde" butonu: masada iki yatık sikke + yaslanmış bir sikke. */
export default function CoinCheckoutButton({ text, disabled, busy, className, onClick }: Props) {
  // Aynı sayfada drawer ve sepet sayfası birlikte render olur; gradyan id'leri çakışmasın.
  const uid = useId().replace(/[^a-zA-Z0-9_-]/g, "");
  const id = (name: string) => `${name}-${uid}`;
  const url = (name: string) => `url(#${id(name)})`;
  const [label, smile] = splitSmile(text);
  ...
  return (
    ...
    >
      ...
      <span className="coin-pay__label">
        {label}
        {smile && <span className="coin-pay__smile">{smile}</span>}
      </span>
    </button>
  );
}
